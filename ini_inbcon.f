...
cmpk amplify the anomaly
c              sstan3(i,j,it) = 1.5d0*r4inp(i,j)    
            enddo
          enddo
...
C mpk BEGIN
C     Read observed additional heating prepared by Gereon Gallon at GEOMAR
C
c     read v and d, and send to dummy4
c11Apr2015       do nvar = 1,2 
c        do l = 1,8 
c         read (80) ((r4inp(i,j),i=1,ix),j=1,il)
c         do j = 1,il
c          do i = 1,ix         
c           if(r4inp(i,j).ne.0.) print*,r4inp(i,j) 
c          enddo
c         enddo
c        enddo
c       enddo
c
cmpk Read climatoglical heating produced by SPEEDY 
         do it = 1,12
          do l = 1,8
           read (81) ((r4inp(i,j),i=1,ix),j=1,il)
           do j = 1,il
             do i = 1,ix
               heat(i,j,9-l,it) = r4inp(i,j)
             enddo
            enddo
           enddo
          enddo
c mpk Read anomaly heating file. 
           do l = 1, 8
            read (80) ((r4inp(i,j),i=1,ix),j=1,il)
            do j = 1,il
             do i = 1,ix
c mpk  6 here means amplifying the magnitude read in
              heat(i,j,9-l,12) = heat(i,j,9-l,12)+6.0*r4inp(i,j)
              heat(i,j,9-l,1) = heat(i,j,9-l,1)+6.0*r4inp(i,j)
              heat(i,j,9-l,2) = heat(i,j,9-l,2)+6.0*r4inp(i,j)
c              if(abs(r4inp(i,j)).gt.5e-6) print*,r4inp(i,j)
             enddo
            enddo
           enddo
C mpk END
