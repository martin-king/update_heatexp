...
cfk#endif
      ...
      DO K=2,NLEV
       DO J=1,NGP
c mpk heat        
c mpk        TTEND(J,K)=TTEND(J,K)+TT_CNV(J,K)+TT_LSC(J,K)
c mpk no using TT_CNV, but add heat1 below
...
C mpk-- BEGIN 
C  Add observed diabatic heating to temp tendency
C
      ...
      DO K=1,NLEV
        DO J=1,NGP
         JLAT=(J+NLON-1)/NLON
	 J0=J-(JLAT-1)*NLON
c          IF(JLAT .LT. 34 .AND. JLAT .GT. 15 .AND. 
c     &       J0 .LT. 75  ) THEN
c          IF(JLAT .LT. 34 .AND. JLAT .GT. 15 .AND. 
c     &       K .EQ. 3  ) THEN
          IF(JLAT .GT. 17 .AND. JLAT .LT. 32 )THEN
            TTEND(J,K)=TTEND(J,K)+HEAT1(J0,JLAT,K)
          ELSE
            TTEND(J,K)=TTEND(J,K)+TT_CNV(J,K)
c            if(heat1(j0,jlat,k).lt.1e-6)print*,'Does not work'
          ENDIF
        ENDDO
      ENDDO
C mpk-- END
